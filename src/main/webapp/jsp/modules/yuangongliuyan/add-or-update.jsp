...
<body>
							...
							<form id="addOrUpdateForm">
								<div class="form-row">
									<input id="updateId" name="id" type="hidden">
										...
										<label>留言标题</label>
										<input id="liuyanbiaoti" name="liuyanbiaoti" class="form-control" placeholder="留言标题">
									</div>
													<div class="form-group col-md-6">
										<label>留言日期</label>
										<div id="liuyanriqiDate" class="input-append date form_datetime">
											<input id="liuyanriqi-input" name="liuyanriqi" size="15" type="text" readonly>
										...
										<input id="lianxidianhua" name="lianxidianhua" class="form-control" placeholder="联系电话">
																							...
																							<label>留言内容</label>
																							<input id="liuyanneirongupload" name="file" type="file">
	...
	<script type="text/javascript" charset="utf-8" src="${pageContext.request.contextPath}/resources/js/datetimepicker/bootstrap-datetimepicker.min.js"></script>
		...
		// 文件上传
		function upload(){
																			$('#liuyanneirongupload').fileupload({
						url: baseUrl + 'file/upload',
						headers:{ token: window.sessionStorage.getItem("token")},
						dataType: 'json',
						done: function (e, data) {	
							UE.getEditor('liuyanneirongEditor').execCommand('insertHtml','<img src=\"'+ baseUrl +'upload/'+ data.result.file + '\" width=900 height=560>');					
						}
					});
																																																																								}  
		...
		function submit() {
			if(validform() ==true && compare() == true){
				...
				httpJson("yuangongliuyan/"+urlParam,"POST",data,(res)=>{
					if(res.code == 0){
		...
		// 获取富文本框内容
		function getContent(){
																if(UE.getEditor('liuyanneirongEditor').hasContents()){
					$('#liuyanneirong-input').attr('value',UE.getEditor('liuyanneirongEditor').getPlainTxt());
				}
																																																														}
		...
		function dataBind(){
			$("#updateId").val(ruleForm.id);	
												$("#liuyanbiaoti").val(ruleForm.liuyanbiaoti);	
																$("#liuyanneirong").val(ruleForm.liuyanneirong);	
																$("#liuyanriqi-input").val(ruleForm.liuyanriqi);
				...
				http(crossTableName + "/info/" + crossTableId,"GET",{},(res)=>{
					if(res.code == 0){
						...
						$('#updateId').val(crossTableId);
												if(res.data != null && res.data != '' && res.data.liuyanbiaoti != null && res.data.liuyanbiaoti != ''){

															$("#liuyanbiaoti").val(res.data.liuyanbiaoti);
													}
												if(res.data != null && res.data != '' && res.data.liuyanneirong != null && res.data.liuyanneirong != ''){

															$("#liuyanneirong").val(res.data.liuyanneirong);
													}
												if(res.data != null && res.data != '' && res.data.liuyanriqi != null && res.data.liuyanriqi != ''){

															$("#liuyanriqi-input").val(res.data.liuyanriqi);
								$('#liuyanriqiFileName').val(res.data.liuyanriqi)
													}
