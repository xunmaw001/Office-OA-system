...
<body>
		...
		<div id="content">
				...
				<div class="container mt-0">
					<div class="row breadcrumb-bar">
						<div class="col-md-6">
							<h3 class="block-title">员工留言管理</h3>
						</div>
						<div class="col-md-6">
							<ol class="breadcrumb">
								...
								<li class="breadcrumb-item active">员工留言列表</li>
							</ol>
						</div>
					</div>
				</div>
				...
				<div class="row">
					...
					<div class="col-md-12">
						<div class="widget-area-2 lochana-box-shadow">
							<h3 class="widget-title">员工留言列表</h3>							
							<div class="table-responsive mb-3">
								<div class="col-sm-12">
																																									<label>
												留言标题
												<input type="text" id="liuyanbiaotiSearch" class="form-control form-control-sm" placeholder="" aria-controls="tableId">
											</label>
																																																																																																																							<label>
												员工工号
											...
											<th class="no-sort" style="min-width: 35px;">
												<div class="custom-control custom-checkbox">
													...
													<label class="custom-control-label" for="select-all"></label>
																																						...
																																						<th onclick="sort('liuyanbiaoti')">留言标题<i id="liuyanbiaotiIcon" class="fa fa-sort"></i></th>
																																																			<th onclick="sort('liuyanneirong')">留言内容<i id="liuyanneirongIcon" class="fa fa-sort"></i></th>
																																																			<th onclick="sort('liuyanriqi')">留言日期<i id="liuyanriqiIcon" class="fa fa-sort"></i></th>
	...
	<script>
		...
		function search(){
			searchForm = { key: ""};
													if($('#liuyanbiaotiSearch').val() != null && $('#liuyanbiaotiSearch').val() != ''){
					searchForm.liuyanbiaoti ="%" +  $('#liuyanbiaotiSearch').val() + "%";
				}
        ...
        function getDataList() {
			http("yuangongliuyan/page","GET",{
				page: pageIndex,
				limit: pageSize,
				sort: sortColumn,
				order: sortOrder,
																						liuyanbiaoti : searchForm.liuyanbiaoti,	
		...
		function setDataRow(item,number){
			...
			row.appendChild(checkbox)
							...
							var liuyanbiaotiCell  = document.createElement('td');

												 liuyanbiaotiCell.innerHTML = item.liuyanbiaoti;

				
									row.appendChild(liuyanbiaotiCell);
				            				var liuyanneirongCell  = document.createElement('td');

												  //如果图文详情包含图片
				  if(item.liuyanneirong && item.liuyanneirong.indexOf('img') != -1){
					//暂时只考虑图片+文字 和 文字+图片的情况
					var beginIndex = item.liuyanneirong.indexOf('<img');
					var endIndex = item.liuyanneirong.indexOf('>');				
					liuyanneirongCell.innerHTML = item.liuyanneirong.substring(beginIndex, endIndex+1).replace("img","img width='100' height='100'");				
				  }else{
				  	 if(item.liuyanneirong != null && item.liuyanneirong != "" && item.liuyanneirong.length >= 11){
				  		...
				  		liuyanneirongCell.innerHTML = item.liuyanneirong;
									...
									row.appendChild(liuyanneirongCell);
				            				var liuyanriqiCell  = document.createElement('td');

												  liuyanriqiCell.innerHTML = item.liuyanriqi;

				
									row.appendChild(liuyanriqiCell);
		...
		function shDialog(id){
			$("#shId").attr("value",id);
			http("yuangongliuyan/info/" + id,"GET",{},(res)=>{
				if(res.code == 0){
		...
		function sh(){
			...
			httpJson("yuangongliuyan/update","POST",data,(res)=>{
				if(res.code == 0){
        ...
        function remove(id) { 
            ...
            if (mymessage == true) {
				...
				httpJson("yuangongliuyan/delete","POST",paramArray,(res)=>{
					if(res.code == 0){
						getDataList();		
						alert('删除成功');
					}
		...
		function getSum(colName){
			http("yuangongliuyan"+colName,"GET",{
		...
		function crossTable(id,crossTableName){
			...
			window.sessionStorage.setItem('crossTableName',"yuangongliuyan");
